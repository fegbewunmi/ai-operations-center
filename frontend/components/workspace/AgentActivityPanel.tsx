  ...
  return (
    <Panel title="Agent activity" className="flex-1 min-h-0 flex flex-col">
      <div className="flex-1 min-h-0 flex flex-col">
        <div className="border-b border-border px-3 py-2 text-[12px] text-fg">
        ...
        {byNode.size > 0 && (
          <table className="w-full text-[11px] border-b border-border">
            <thead>
              <tr className="text-left text-fg-faint">
                <th className="px-3 py-1.5 font-medium">Node</th>
                <th className="px-3 py-1.5 font-medium text-right">Calls</th>
            ...
            <tbody>
              {[...byNode.entries()].map(([node, stats]) => (
                <tr key={node} className="border-t border-border/60">
                  <td className="px-3 py-1 text-fg">{node}</td>
                  <td className="px-3 py-1 text-right text-fg-muted tabular-nums">{stats.calls}</td>
                  <td className="px-3 py-1 text-right text-fg-muted tabular-nums">
                    {(stats.inTok + stats.outTok).toLocaleString()}
                  </td>
                  <td className="px-3 py-1 text-right text-fg-muted tabular-nums">${stats.cost.toFixed(4)}</td>
                </tr>
              ))}
            </tbody>
          </table>
              ...
              {[...events].reverse().map((e, i) => (
                <li key={i} className="px-3 py-2">
                  <div className="flex items-center justify-between gap-2">
                    <span className="text-[11px] font-medium text-fg">{label(e.source)}</span>
                    <span className="text-[10px] text-fg-faint tabular-nums">{formatRelativeTime(e.timestamp)}</span>
                  </div>
                  <p className="mt-0.5 text-[11px] text-fg-muted leading-snug">{e.description}</p>
                </li>
              ))}
            </ul>
          )}
