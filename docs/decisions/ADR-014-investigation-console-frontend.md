...
**Correction (2026-09-01):** this originally claimed challenge-resume was *not* affected by the L3 dead-end bug, "confirmed by the fact that replayed challenge-resumes do visibly re-invoke the planner." That claim was wrong and, as far as can be reconstructed, was never actually verified — a minimal repro against the real `aupdate_state(phase="planning") + ainvoke(None, config)` pattern on a genuinely-finished thread showed the node executes zero times on "resume"; nothing was re-invoked. Challenge-resume had the identical dead end as L3 approval, just reached via a different phase-gate condition. Both were fixed together in `ADR-015`, which also corrects the record here: don't trust an unverified claim in an ADR over re-checking the actual mechanism, including ones written earlier in this same document.
